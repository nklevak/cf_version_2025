...
2) For the effect of task switching x prior performance on future performance, we used an effect size of 0.3 to be conservative (although our effect size in the pilot study was larger). Using this, and a slightly simplified glmer binomial model than we used to get our result in the extended abstract, we got that a sample size of 5 or larger should be sufficient.
...
- looking at our previous pilot with 84 participants, the bottom quartile of response time per trial with non-excluded data was: ______ so we will use that as a lower boundary for exclusion. Specifically, we will exclude any participants whose mean trial response time was below the bottom quartile.
